...
class GetModel{
    ...
    static public function getBetweenRelData($rel, $type, $linkTo, $between1, $between2,  $filterTo, $inTo, $not, $orderBy, $orderMode, $startAt, $endAt, $select){

        if(!is_numeric($inTo)){
            $neinTo = "";
            $stringSeparado = explode(",", $inTo);
            foreach($stringSeparado as $key => $value){  
                $neinTo .= "'".$value."',";
            }
            $inTo=trim($neinTo, ',');
            if($not == null){
    ...
    /* Get Petition having  */
    static public function getHaving($table, $linkTo, $equalTo, $groupBy, $orderBy, $orderMode, $startAt, $endAt, $select){
        $resutl = [];
        $numberStock = 0;
        $dateUpdate= "date_update_stock";
        $arrayGoupBy = explode(",", $groupBy);
        try{
            foreach($arrayGoupBy as $key => $value){
                if($orderBy!=null && $orderMode!=null && $startAt==null && $endAt==null){
                    $stmt = Conection::connect()->prepare("SELECT $value FROM $table WHERE $linkTo=$equalTo GROUP BY $value HAVING COUNT(*) > 1 ORDER BY $orderBy $orderMode");
                } else if($orderBy!=null && $orderMode!=null && $startAt!=null && $endAt!=null){
                    $stmt = Conection::connect()->prepare("SELECT $value FROM $table WHERE $linkTo=$equalTo GROUP BY $value HAVING COUNT(*) > 1 ORDER BY $orderBy $orderMode LIMIT $startAt, $endAt");
                }else{
                    $stmt = Conection::connect()->prepare("SELECT $value FROM $table WHERE $linkTo=$equalTo GROUP BY $value HAVING COUNT(*) > 1");
                }
                $stmt -> execute();
                $stmt = $stmt ->fetchAll(PDO::FETCH_CLASS);
                foreach( $stmt as $value2 ){
                    $resutl[] = $value2;
                }
            }
            $stmt = Conection::connect()->prepare("SELECT $select FROM $table WHERE $linkTo=$equalTo");
            $stmt -> execute();
            $stmt = $stmt ->fetchAll(PDO::FETCH_CLASS);
            foreach( $stmt as $value2 ){
                $numberStock += $value2->number_stock;
            }
            $resutl[] = (object) ["number_stock" => $numberStock];
            $stmt = Conection::connect()->prepare("SELECT $dateUpdate FROM $table WHERE $dateUpdate  = (SELECT MAX($dateUpdate) FROM $table WHERE $linkTo = $equalTo) AND $linkTo = $equalTo;");
            $stmt -> execute();
            $stmt = $stmt ->fetchAll(PDO::FETCH_CLASS);
            $resutl[] = (object) ["date_update_stock" => $stmt[0]->date_update_stock];
